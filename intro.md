...
* `Cmd-T`/`Ctrl-T`: Show file tree. Keys used with tree active:
    * `Up`: Move up in the tree
    * `Down`: Move down in the tree
    * `Left`: Collapse node
    * `Right`: Expand node
    * `Enter`: Open file
* `Command-F`/`Ctrl-F`: Find within file, after pressing `Enter` to select thie first match, use `Command-G`/`Ctrl-G` to find next match, or `Ctrl-Alt-F` to put cursors on all.
